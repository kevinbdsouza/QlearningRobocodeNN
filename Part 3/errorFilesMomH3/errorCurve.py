...
for nRun in range(0,9): 
	errorfile = open("/home/kevin/Learning systems/Part 3/errorFilesMomH3/errorFileMom"+ str((nRun+1)*0.1) +".txt", "r") 
...
for nRun in range(0,9): 
	errorfile = open("/home/kevin/Learning systems/Part 3/errorFilesMomH3/errorFileMomVal"+ str((nRun+1)*0.1) +".txt", "r") 
	error =  errorfile.readlines()
	epochValues.append(len(error))
	valError.append(error[4]) 
	errorfile.close()

moment = np.arange(0.1,1,0.1)
print(len(moment))
plt.xlabel('Momentum')
plt.ylabel('RMS error')
plt.title('RMS error vs Momentum')
lineTrain, =plt.plot(moment, trainError, c='r', label="Training Error", linewidth=2.0)
lineVal, = plt.plot(moment, valError, c='g', label="Validation Error", linewidth=2.0)
...
lineTrain, =plt.plot(hidden, trainErrorMov, c='r', label="Training Error", linewidth=2.0)
lineVal, = plt.plot(hidden, valErrorMov, c='g', label="Validation Error", linewidth=2.0)
plt.legend(handler_map={lineTrain: HandlerLine2D(numpoints=4)})
plt.show()
'''
